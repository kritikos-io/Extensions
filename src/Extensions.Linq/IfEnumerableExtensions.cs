...
public static class IfEnumerableExtensions
{
  ...
  public static IEnumerable<TSource> TakeIf<TSource>(this IEnumerable<TSource> source, bool condition, int take)
    => condition
      ? source.Take(take)
      : source;
  ...
  /// <exception cref="ArgumentNullException"><paramref name="source"/> was <see langword="null"/>.</exception>
  public static IEnumerable<TSource> SkipIf<TSource>(this IEnumerable<TSource> source, bool condition, int skip)
    => condition
      ? source.Skip(skip)
      : source;
}
